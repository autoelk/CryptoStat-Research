...
//
// This C source file is copyright (C) 2017 by Alan Kaminsky. All rights
...
int throwCudaException
	...
	switch (rv)
		...
		case CUDA_ERROR_UNSUPPORTED_LIMIT:
			name = "edu/rit/gpu/UnsupportedLimitCudaException";
			break;
		default:
...
/*
 * Class:     edu_rit_gpu_Cuda
 * Method:    cuCtxSetLimit
 * Signature: (JIJ)V
 */
JNIEXPORT void JNICALL Java_edu_rit_gpu_Cuda_cuCtxSetLimit
	(JNIEnv *env,
	 jclass cls,
	 jlong ctx,
	 jint limit,
	...
	if (! exc)
		{
		rv = cuCtxSetLimit (limit, jlong_to_csize_t (value));
		exc = throwCudaException (env, rv, "calling cuCtxSetLimit()");
		rv = cuCtxPopCurrent (&context);
		if (! exc)
			throwCudaException (env, rv, "calling cuCtxPopCurrent()");
		}
...
/*
 * Class:     edu_rit_gpu_Cuda
 * Method:    cuCtxGetLimit
 * Signature: (JI)J
 */
JNIEXPORT jlong JNICALL Java_edu_rit_gpu_Cuda_cuCtxGetLimit
	(JNIEnv *env,
	 jclass cls,
	 jlong ctx,
	 jint limit)
	...
	if (! exc)
		{
		rv = cuCtxGetLimit (&value, limit);
		exc = throwCudaException (env, rv, "calling cuCtxGetLimit()");
		rv = cuCtxPopCurrent (&context);
		if (! exc)
			throwCudaException (env, rv, "calling cuCtxPopCurrent()");
		}
	return csize_t_to_jlong (value);
	}
