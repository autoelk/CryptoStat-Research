 ...
 * <LI>{@link #getDeviceNumber() getDeviceNumber()}
 * <LI>{@link #getDeviceName() getDeviceName()}
 ...
 * <LI>{@link #setThreadStackSize(long) setThreadStackSize()},
 *     {@link #getThreadStackSize() getThreadStackSize()}
 * <LI>{@link #setPrintfFifoSize(long) setPrintfFifoSize()},
 *     {@link #getPrintfFifoSize() getPrintfFifoSize()}
 * <LI>{@link #setMallocHeapSize(long) setMallocHeapSize()},
 *     {@link #getMallocHeapSize() getMallocHeapSize()}
 * </UL>
 * <P><LI>
 ...
 * </UL>
 * <P><LI>
 * Create a dynamically allocated struct, struct array, or struct matrix in GPU
 * memory &mdash;
 * <UL>
 * <LI>{@link #getStructVbl(Class) getStructVbl()}
 * <LI>{@link #getStructArray(Class,int) getStructArray()}
 * <LI>{@link #getStructMatrix(Class,int,int) getStructMatrix()}
 * </UL>
 * <P><LI>
...
// Exported class data members.

	/**
	 * Debug flag. If true, when a Gpu object is created, information about the
	 * device is printed on the standard error. Default false.
	 */
...
// Hidden constructors.
	 ...
	 *
	 * @param  ctxSched  Context scheduling flag.
	...
	private Gpu
		...
		ctx = Cuda.cuCtxCreate (ctxSched.value, device);
		if (debug)
			{
...
// Exported operations.
	...
	/**
	 * Construct a new Gpu object. The next available GPU device is used. A
	 * context scheduling flag of {@link CtxSched#CU_CTX_SCHED_BLOCKING_SYNC
	 * CU_CTX_SCHED_BLOCKING_SYNC} is used.
	 *
	 * @return  Gpu object.
	 *
	...
	public static synchronized Gpu gpu()
		{
		return Gpu.gpu (CtxSched.CU_CTX_SCHED_BLOCKING_SYNC);
		}

	/**
	 * Construct a new Gpu object. The next available GPU device is used. The
	 * given context scheduling flag is used.
	 *
	 * @param  ctxSched  Context scheduling flag.
	 *
	 * @return  Gpu object.
	...
	public static synchronized Gpu gpu
		...
		return new Gpu (ctxSched, allowedDeviceNumbers[inUseDevices++]);
		}
	 ...
	 *
	 * @param  value  Thread stack size limit.
	 *
	 * @exception  GpuException
	 *     (unchecked exception) Thrown if a GPU error occurred.
	 */
	public void setThreadStackSize
		(long value)
		{
		Cuda.cuCtxSetLimit (ctx, Cuda.J_CU_LIMIT_STACK_SIZE, value);
		}

	/**
	 * Get this GPU's thread stack size limit.
	 *
	 * @return  Thread stack size limit.
	 *
	 * @exception  GpuException
	 *     (unchecked exception) Thrown if a GPU error occurred.
	 */
	public long getThreadStackSize()
		{
		return Cuda.cuCtxGetLimit (ctx, Cuda.J_CU_LIMIT_STACK_SIZE);
		}

	/**
	 * Set this GPU's <TT>printf()</TT> FIFO size limit.
	 *
	 * @param  value  <TT>printf()</TT> FIFO size limit.
	 *
	 * @exception  GpuException
	 *     (unchecked exception) Thrown if a GPU error occurred.
	 */
	public void setPrintfFifoSize
		(long value)
		{
		Cuda.cuCtxSetLimit (ctx, Cuda.J_CU_LIMIT_PRINTF_FIFO_SIZE, value);
		}

	/**
	 * Get this GPU's <TT>printf()</TT> FIFO size limit.
	 *
	 * @return  <TT>printf()</TT> FIFO size limit.
	 *
	 * @exception  GpuException
	 *     (unchecked exception) Thrown if a GPU error occurred.
	 */
	public long getPrintfFifoSize()
		{
		return Cuda.cuCtxGetLimit (ctx, Cuda.J_CU_LIMIT_PRINTF_FIFO_SIZE);
		}

	/**
	 * Set this GPU's <TT>malloc()</TT> heap size limit.
	 *
	 * @param  value  <TT>malloc()</TT> heap size limit.
	 *
	 * @exception  GpuException
	 *     (unchecked exception) Thrown if a GPU error occurred.
	 */
	public void setMallocHeapSize
		(long value)
		{
		Cuda.cuCtxSetLimit (ctx, Cuda.J_CU_LIMIT_MALLOC_HEAP_SIZE, value);
		}

	/**
	 * Get this GPU's <TT>malloc()</TT> heap size limit.
	 *
	 * @return  <TT>malloc()</TT> heap size limit.
	 *
	 * @exception  GpuException
	 *     (unchecked exception) Thrown if a GPU error occurred.
	 */
	public long getMallocHeapSize()
		{
		return Cuda.cuCtxGetLimit (ctx, Cuda.J_CU_LIMIT_MALLOC_HEAP_SIZE);
		}
	...
	/**
	 * Create a new dynamically allocated GPU struct matrix. The entire GPU
	 * matrix is mirrored in the CPU. The matrix elements in CPU memory, namely
	 * the {@link GpuStructMatrix#item item} field's elements, are initialized
	 * to null. <I>The matrix elements in GPU memory are not initialized.</I>
	 *
	 * @param  <T>   Java data type. Class <TT>T</TT> must extend class
	 *               {@linkplain Struct Struct}.
	 * @param  type  Class object for Java data type.
	 ...
	 *     (unchecked exception) Thrown if a GPU error occurred.
	 *
	 * @see  #getStructMatrix(Class,int,int,int,int)
	 */
	public <T extends Struct> GpuStructMatrix<T> getStructMatrix
		(Class<T> type,
		 int rows,
		 int cols)
		{
		return getStructMatrix (type, rows, cols, rows, cols);
		}
	 ...
	 * portion is mirrored in the CPU. If <TT>cpurows</TT> = 0 or
	 * <TT>cpucols</TT> = 0, then none of the matrix is mirrored in the CPU. The
	 * matrix elements in CPU memory, namely the {@link GpuStructMatrix#item
	 * item} field's elements, are initialized to null. <I>The matrix elements
	 * in GPU memory are not initialized.</I>
	 *
	 * @param  <T>      Java data type. Class <TT>T</TT> must extend class
	 *                  {@linkplain Struct Struct}.
	 ...
	 *     (unchecked exception) Thrown if a GPU error occurred.
	 *
	 * @see  #getStructMatrix(Class,int,int)
	 */
	public <T extends Struct> GpuStructMatrix<T> getStructMatrix
		(Class<T> type,
		 int rows,
		...
		if (rows < 1)
			throw new IllegalArgumentException (String.format
				("Gpu.getStructMatrix(): rows = %d illegal", rows));
		if (cols < 1)
			throw new IllegalArgumentException (String.format
				("Gpu.getStructMatrix(): cols = %d illegal", cols));
		if (0 > cpurows || cpurows > rows)
			throw new IllegalArgumentException (String.format
				("Gpu.getStructMatrix(): cpurows = %d illegal", cpurows));
		if (0 > cpucols || cpucols > cols)
			throw new IllegalArgumentException (String.format
				("Gpu.getStructMatrix(): cpucols = %d illegal", cpucols));
		return new GpuStructMatrix (this, rows, cols, cpurows, cpucols, type);
		}
	...
	public String toString()
		{
		return String.format ("GPU %d, %s, compute capability %d.%d, %d MPs",
			devnum, name, major, minor, mpcount);
		}
...
//			{
//			System.out.printf ("Gpu.finalize()%n");
...
//		System.out.printf ("cache config = %s%n", gpu.getCacheConfig());
//		System.out.printf ("thread stack size = %d%n", gpu.getThreadStackSize());
//		gpu.setThreadStackSize (2048);
//		System.out.printf ("thread stack size = %d%n", gpu.getThreadStackSize());
//		System.out.printf ("printf fifo size = %d%n", gpu.getPrintfFifoSize());
//		gpu.setPrintfFifoSize (2000000);
//		System.out.printf ("printf fifo size = %d%n", gpu.getPrintfFifoSize());
//		System.out.printf ("malloc heap size = %d%n", gpu.getMallocHeapSize());
//		gpu.setMallocHeapSize (16000000);
//		System.out.printf ("malloc heap size = %d%n", gpu.getMallocHeapSize());
//		}
