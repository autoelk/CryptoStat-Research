...
// Unit:    Class edu.rit.pj2.example.CCASeq
//
 ...
 * file.
 * <P>
 * Usage: <TT>java pj2 edu.rit.pj2.example.CCASeq <I>N</I> <I>S</I> <I>A</I>
...
// Main program.
	...
	public void main
		...
		for (int s = 0; s < S; ++ s)
			...
			for (int i = 0; i < N; ++ i)
				{
				next[i] .assign (T.assign (A) .multiply (current[(i-1+N)%N]));
				next[i] .add (T.assign (B) .multiply (current[i]));
				next[i] .add (T.assign (C) .multiply (current[(i+1)%N]));
...
// Hidden operations.
	...
	private static void usage()
		{
		System.err.println ("Usage: java pj2 edu.rit.pj2.example.CCASeq <N> <S> <A> <B> <C> <D> <file>");
