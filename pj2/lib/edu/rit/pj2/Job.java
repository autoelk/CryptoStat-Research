 ...
 * If code in a job or task throws an exception that propagates out of the job's
 * or task's <TT>main()</TT> method, the <TT>pj2</TT> program prints an
 * exception stack trace and terminates. The exception's detail message can
 * explain why the job or task terminated.
 * <P>
 * If a job or task needs to terminate other than by returning from the job's or
 * task's <TT>main()</TT> method, but without printing an exception stack trace,
 * the job or task should call the {@link #terminate(int) terminate()} method.
 * Code in a job or task must <B><I>not</I></B> call the <TT>System.exit()</TT>
 * method; doing so screws up the Parallel Java 2 middleware.
 *
...
// Exported operations.
	 ...
	 *
	 * @param  safe  True to use multiple thread safe console output streams,
	 *               false otherwise.
	 *
	 * @return  Exit code to be passed back to the operating system.
	 *
	 * @exception  Exception
	...
	public final int execute
		...
		try
			...
			// Initialize job's distributed work queue if any.
			if (jobWorkQueue != null)
				jobWorkQueue.putInitialTuples();
			}
...
//			errorMsg = null;
		...
		// A task in the job was explicitly terminated.
		else if (excThrown instanceof TerminateException)
			{
			errorMsg = null;
			printFinalDebugs();
			return ((TerminateException)excThrown).exitCode;
			}

		// A task in the job threw an exception; re-throw it with an error
		// message,
		else
			{
	...
	/**
	 * Print final debug messages for this job.
	 */
	private void printFinalDebugs()
		{
	...
	/**
	 * Terminate this job. The given exit code is returned to the operating
	 * system.
	 *
	...
	public static void terminate
		(int exitCode)
		{
		throw new TerminateException (exitCode);
		}

	/**
	 * Get a reference to this job's distributed work queue. If
	 * <TT>numWorkers</TT> is {@link #DEFAULT_WORKERS}, then there will be one
	 * worker task. For further information, see class {@linkplain
	 * JobWorkQueue}.
	 *
	 * @param  <W>         Data type of the work items. The work item class
	 *                     must be a subclass of class {@linkplain Tuple}.
	 * @param  Wclass      Class object for the work items.
	 ...
	 *
	 * @exception  NullPointerException
	 *     (unchecked exception) Thrown if <TT>Wclass</TT> is null.
	 * @exception  IllegalArgumentException
	 *     (unchecked exception) Thrown if <TT>numWorkers</TT> &lt; 1.
	 * @exception  IllegalStateException
	 *     (unchecked exception) Thrown if <TT>getJobWorkQueue()</TT> was
	 *     previously called in this job process.
	 */
	public <W extends Tuple> JobWorkQueue<W> getJobWorkQueue
		(Class<W> Wclass,
		 int numWorkers)
		{
		if (Wclass == null)
			throw new NullPointerException
				("Job.getJobWorkQueue(): Wclass is null");
		if (jobWorkQueue != null)
			throw new IllegalStateException
				("Job.getJobWorkQueue() was previously called in this process");
		jobWorkQueue = new JobWorkQueue (this, numWorkers);
		return (JobWorkQueue<W>) jobWorkQueue;
		}
	...
	private synchronized void fireRule
		...
		rule.taskGroup.forEachItemDo (new Action<TaskSpec>()
			...
			public void run (TaskSpec taskSpec)
				...
				info.moreData = extra;
				info.debugGpu = taskDebug (Debug.gpu, info);
