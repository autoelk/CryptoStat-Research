 ...
 * If code in a task throws an exception that propagates out of the task's
 * <TT>main()</TT> method, the {@link pj2 pj2} program prints an exception stack
 * trace and terminates. The exception's detail message can explain why the task
 * terminated.
 * <P>
 * If a task needs to terminate other than by returning from the task's
 * <TT>main()</TT> method, but without printing an exception stack trace, the
 * task should call the {@link #terminate(int) terminate()} method. Code in a
 * task must <B><I>not</I></B> call the <TT>System.exit()</TT> method; doing so
 * screws up the Parallel Java 2 middleware.
 *
...
// Hidden data members.
	...
	JobWorkQueue jobWorkQueue;       // null if not part of a job
...
// Exported operations.
	...
	/**
	 * Get a reference to this task's job's distributed work queue. This task
	 * must be part of a {@linkplain Job}. For further information, see class
	 * {@linkplain JobWorkQueue}.
	 *
	 * @param  <W>     Data type of the work items. The work item class must be
	 *                 a subclass of class {@linkplain Tuple}.
	 ...
	 * @exception  IllegalStateException
	 *     (unchecked exception) Thrown if this task is not part of a job.
	 *     Thrown if <TT>getJobWorkQueue()</TT> was previously called in this
	 *     task process.
	 */
	public <W extends Tuple> JobWorkQueue<W> getJobWorkQueue
		...
		if (Wclass == null)
			throw new NullPointerException
				("Task.getJobWorkQueue(): Wclass is null");
		if (job == null)
			throw new IllegalStateException
				("Task.getJobWorkQueue(): Task is not part of a job");
		if (jobWorkQueue != null)
			throw new IllegalStateException
				("Task.getJobWorkQueue() was previously called in this process");
		jobWorkQueue = new JobWorkQueue (this);
		return (JobWorkQueue<W>) jobWorkQueue;
		}

	/**
	 * Terminate this task. The given exit code is returned to the operating
	 * system.
	 *
