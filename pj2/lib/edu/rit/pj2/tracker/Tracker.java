 ...
 * <TT><I>name</I></TT> as the name of the node. Specify <TT><I>cores</I></TT>
 * as the number of CPU cores in the node (&ge; 1). Specify <TT><I>gpus</I></TT>
 * either as the number of GPU accelerators in the node (&ge; 0), or as a
 * parenthesized comma-separated list of GPU IDs in the node. If omitted, tasks
 * scheduled by the Tracker will run in a Backend process on a separate node;
 * this is intended for a multi-node cluster parallel computer.
 * </UL>
 * <P>
 * <B>Job lifecycle.</B>
	...
	public void launcherStarted
		...
		synchronized (lock)
			...
			for (int i = 0; i < info.gpusTotal; ++ i)
				info.devnumsAvailable.addLast (id[i]);
