 ...
 * <P><LI>
 * <TT>gpus=<I>gpus</I></TT> -- If <I>gpus</I> is a single integer, the
 * Launcher's node has the given number of GPU accelerators. If <I>gpus</I> is a
 * parenthesized comma-separated list of one or more integers, the Launcher's
 * node has GPU accelerators with the given GPU IDs. If omitted, the default is
 * 0 (no GPU accelerators).
 * <P><LI>
...
// Hidden data members.
	...
	private static String cores = "1";
	private static String gpus = "0";
	private static String command = "java";
	private static String[] splitCommand = new String[] { "java" };

	private static NodeProperties props;
	...
	public static void main
		...
		for (int i = 0; i < args.length; ++ i)
			...
			if (args[i].startsWith ("tracker="))
				...
				cores = args[i].substring (6);
			else if (args[i].startsWith ("gpus="))
				gpus = args[i].substring (5);
		...
		try
			...
			{
			usage (String.format ("cores=%s illegal", cores));
			}
		try
			{
			props.gpus (gpus);
			...
			{
			usage (String.format ("gpus=%s illegal", gpus));
			}
		...
		try
			...
			logger.log (String.format
				("Launcher started, tracker=%s:%d name=%s cores=%s gpus=%s command=\"%s\"",
			...
			if (name == null)
				props.nodeName (trackerProxy.localHost());
			...
			tracker.launcherStarted (launcher, props);
