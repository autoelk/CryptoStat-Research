 ...
 * of a {@linkplain Streamable} object, and also keeps a reference to the
 * object. If the same object is read by {@link #readReference()
 * readReference()} again, just a reference to the previous object is returned.
 ...
 * Serialization, {@link #readObject() readObject()} will work both on
 * {@linkplain Streamable} objects and on {@linkplain java.io.Serializable
 * Serializable} objects. In the latter case, a byte array is read, and the byte
 * array is converted to an object using Java Object Serialization. ({@link
 * #readReference() readReference()} does not support this.)
 * <P>
...
// Exported operations.
	 ...
	 * <P>
	 * <I>Note:</I> <TT>readReference()</TT> does not support reading
	 ...
	 * @exception  IOException
	 *     Thrown if an I/O error occurred. Thrown if the object could not be
	 *     deserialized. Thrown if the object is not {@linkplain Streamable}.
	 */
	public Object readReference()
		...
		try
			...
			if (v == null)
				...
				if (! info.isStreamable)
					throw new IOException
						("readReference(): object is not Streamable");
				else if (info.ctor != null)
					{
					v = info.ctor.newInstance();
					addObjectToCache (i, v);
					readFields ((Streamable) v);
					}
				else
					{
					throw new NoSuchMethodException (String.format
			...
			{
			throw new DeserializeException
				("InStream.readReference(): Object could not be deserialized",
				 exc);
			}
		}
	 ...
	 *     could not be deserialized. Thrown if the array elements are not
	 *     {@linkplain Streamable}.
	 */
