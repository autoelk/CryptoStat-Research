 ...
 * the fields of a {@linkplain Streamable} object, and also keeps a reference to
 * the object. If the same object is passed to {@link #writeReference(Object)
 * writeReference()} again, just a reference to the previous object is written.
 * Use this method when the reader needs to end up with multiple references to
 * the same object.
 * </UL>
 * <P>
 ...
 * Serialization, {@link #writeObject(Object) writeObject()} will work both on
 * {@linkplain Streamable} objects and on {@linkplain java.io.Serializable
 * Serializable} objects. In the latter case, the object is converted to a byte
 * array using Java Object Serialization, and the byte array is written. ({@link
 * #writeReference(Object) writeReference()} does not support this.)
 * <P>
...
// Exported operations.
	 ...
	 * <P>
	 * <I>Note:</I> <TT>writeReference()</TT> does not support writing
	 * {@linkplain java.io.Serializable Serializable} objects.
	 *
	 * @param  v  Object, or null.
	 *
	 * @exception  IOException
	 *     Thrown if an I/O error occurred. Thrown if <TT>v</TT> is not a
	 *     {@linkplain Streamable} object.
	 */
	public void writeReference
		...
		if (v == null)
			writeUnsignedInt (0);
		else if (! (v instanceof Streamable))
			throw new IOException
				("writeReference(): object is not Streamable");
		else
			{
	 ...
	 *
	 * @exception  IOException
	 *     Thrown if an I/O error occurred. Thrown if the array elements are not
	 *     {@linkplain Streamable} objects.
	 */
