 ...
 * <P><LI>
 * Create the reducer task's configured {@linkplain Customizer}. If no
 * customizer was configured, an instance of the Customizer base class (which
 * does nothing) is created.
 * <P><LI>
 * Receive (key, value) pairs from each {@linkplain MapperTask}. Reduce all the
 * received pairs together into the reducer task's combiner.
 * <P><LI>
 * If the customizer's <TT>comesBefore()</TT> method was overridden, sort the
 * combiner's (key, value) pairs into the order determined by the
 * <TT>comesBefore()</TT> method. Otherwise, leave the combiner's (key, value)
 * pairs in an unspecified order.
 * <P><LI>
 * Call the customizer's <TT>start()</TT> method, passing in any configured
 * argument strings plus the combiner.
 * <P><LI>
 ...
 * </OL>
 * <P><LI>
 * After all the parallel team threads have finished, call the customizer's
 * <TT>finish()</TT> method, passing in the combiner.
 * </OL>
 *
 * @param  <K>  Key data type.
 * @param  <V>  Value data type; must implement interface
