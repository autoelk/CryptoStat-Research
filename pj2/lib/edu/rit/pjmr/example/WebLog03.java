 ...
 * <I>nodes</I> <I>file</I> <I>account</I> [ <I>pattern</I> ]</TT>
 * <P>
 ...
 * given user <I>account</I>, along with the number of requests from each IP
 * address. The IP address is the first item on each web log line. The user
 * account is found between the first <TT>"/~"</TT> and the next <TT>"/"</TT> or
 * whitespace on each web log line. The IP addresses are printed in descending
...
public class WebLog03
	...
	public void main
		...
		for (String node : nodes)
			mapperTask (node)
				...
				.mapper (NT, MyMapper.class, account, pattern);
	...
	private static void usage()
		{
		System.err.println ("Usage: java pj2 [threads=<NT>] edu.rit.pjmr.example.WebLog03 <nodes> <file> <account> [<pattern>]");
		terminate (1);
		}
	...
	private static class MyMapper
		...
		public void map
			(TextId id,       // File name and line number
			 String contents, // Line from file
			 Combiner<IPAddress,LongVbl> combiner)
			{
			if (pattern == null || pattern.matcher (contents) .find())
				{
				...
				if (i != -1)
					...
					while (j < n &&
						...
						! Character.isWhitespace (c))
							++ j;
					if (contents.substring (i + 2, j) .equals (account))
						{
						int k = 0;
						while (k < contents.length() &&
							! Character.isWhitespace (contents.charAt (k)))
								++ k;
						combiner.add (new IPAddress (contents.substring (0, k)),
							ONE);
						}
					}
				}
