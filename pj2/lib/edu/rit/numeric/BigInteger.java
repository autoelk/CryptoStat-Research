...
// Hidden data members.
	...
	private int[] quo;   // Temporary storage used by various operations
...
// Exported operations.
	 ...
	 * string. The string consists of one or more decimal digits (0..9).
	 *
	 * @param  s  String.
	 *
	 * @return  This big integer.
	 ...
	 *     (unchecked exception) Thrown if <TT>s</TT> has no digits or contains
	 *     invalid characters.
	 */
	public BigInteger assign
		(String s)
	 ...
	 * string. The string consists of one or more hexadecimal digits (0..9,
	 * A..F, a..f).
	 *
	 * @param  s  String.
	 *
	...
	public BigInteger assignHex
		...
		if (n == 0)
			throw new NumberFormatException
				("BigInteger.assignHex(): No digits");
		assignZero();
		for (int i = 0; i < n; ++ i)
			{
			leftShift (4);
			value[0] |= charToHexDigit (s.charAt (i));
			}
		clearUnusedBits();
		return this;
		}
	...
	private static int charToHexDigit
		...
		if ('0' <= c && c <= '9')
			...
			throw new NumberFormatException (String.format
				("BigInteger.assignHex(): Illegal digit '%c'", c));
		}
	...
	public BigInteger assignMersenne
		(int n)
		{
		return assignPowerOfTwo (n) .sub (1);
		}
	...
	/**
	 * Set this big integer to its twos complement.
	 *
	 * @return  This big integer.
	 */
	...
	public BigInteger mul
		...
		allocateTmp();
		clearWords (tmp, 0);
		multiplyAtPosition (0, value, (int)n);
		multiplyAtPosition (1, value, (int)(n >>> 32));
		copyWords (tmp, value);
		clearUnusedBits();
		return this;
		}
	...
	public BigInteger mul
		...
		{
		allocateTmp();
		multiply (value, bigint.value);
		return this;
		}

	/**
	 * Multiply array dst by array src, store result back in array dst.
	 */
	private void multiply
		...
		for (int i = 0; i < src.length; ++ i)
			multiplyAtPosition (i, dst, src[i]);
		copyWords (tmp, dst);
		clearUnusedBits (dst);
		}

	/**
	 * Multiply all words of array v by the word n, accumulate result into array
	 * this.tmp starting at position posn.
	 */
	private void multiplyAtPosition
		...
		long sum = 0L;
		for (int s = 0, d = posn; d < v.length; ++ s, ++ d)
			{
			prod = (v[s] & MASK_32) * n_long;
			prod_hi = prod >>> 32;
	...
	/**
	 * Raise this big integer to the given power and assign the result to this
	 * big integer.
	 *
	 * @param  n  Exponent &ge; 0.
	 *
	 * @return  This big integer.
	 *
	 * @exception  IllegalArgumentException
	 ...
	 * <TT>v[1]</TT>, and so on. If <TT>v.length</TT> is not enough bytes to
	 * fill this big integer, the unfilled most significant bits of this big
	 * integer are set to 0. If <TT>v.length</TT> is more bytes than needed to
	 * fill this big integer, the extra bytes are ignored.
	 *
	 * @param  v  Value array.
	 *
	 * @return  This big integer.
	...
	public BigInteger pack
		(byte[] v)
		{
		return pack (v, 0, v.length);
		}

	/**
	 * Pack the value obtained from a portion of the given byte array into this
	 * big integer in little-endian order. Bits 0 through 7 of this big integer
	 * are set to <TT>v[off]</TT>, bits 8 through 15 of this big integer are set
	 * to <TT>v[off+1]</TT>, and so on. If <TT>len</TT> is not enough bytes to
	 * fill this big integer, the unfilled most significant bits of this big
	 * integer are set to 0. If <TT>len</TT> is more bytes than needed to fill
	 * this big integer, the extra bytes are ignored.
	 *
	 * @param  v    Value array.
	 * @param  off  Index of first byte to pack.
	 * @param  len  Number of bytes to pack.
	 *
	 * @return  This big integer.
	 *
	 * @exception  NullPointerException
	...
	public BigInteger pack
		...
		{
		int max = off + len;
		if (off < 0 || len < 0 || max > v.length)
			throw new IndexOutOfBoundsException();
		int src = off;
		int dst = 0;
		while (src < max && dst < value.length)
			{
			value[dst] = v[src++] & 0xff;
			if (src < max) value[dst] |= (v[src++] & 0xff) << 8;
			if (src < max) value[dst] |= (v[src++] & 0xff) << 16;
			if (src < max) value[dst] |= (v[src++] & 0xff) << 24;
			++ dst;
			}
	 ...
	 * <TT>v[1]</TT>, and so on. If <TT>v.length</TT> is not enough shorts to
	 * fill this big integer, the unfilled most significant bits of this big
	 * integer are set to 0. If <TT>v.length</TT> is more shorts than needed to
	 * fill this big integer, the extra shorts are ignored.
	 *
	 * @param  v  Value array.
	 *
	 * @return  This big integer.
	...
	/**
	 * Pack the value obtained from a portion of the given short array into this
	 * big integer in little-endian order. Bits 0 through 15 of this big integer
	 * are set to <TT>v[off]</TT>, bits 15 through 31 of this big integer are
	 * set to <TT>v[off+1]</TT>, and so on. If <TT>len</TT> is not enough shorts
	 * to fill this big integer, the unfilled most significant bits of this big
	 * integer are set to 0. If <TT>len</TT> is more shorts than needed to fill
	 * this big integer, the extra shorts are ignored.
	 *
	 * @param  v    Value array.
	 * @param  off  Index of first short to pack.
	 * @param  len  Number of shorts to pack.
	 *
	 * @return  This big integer.
	 *
	 * @exception  NullPointerException
	...
	public BigInteger pack
		...
		while (src < max && dst < value.length)
			...
			if (src < max) value[dst] |= (v[src++] & 0xffff) << 16;
			++ dst;
			}
		clearWords (value, dst);
		clearUnusedBits();
	 ...
	 * <TT>v[1]</TT>, and so on. If <TT>v.length</TT> is not enough integers to
	 * fill this big integer, the unfilled most significant bits of this big
	 * integer are set to 0. If <TT>v.length</TT> is more integers than needed
	 * to fill this big integer, the extra integers are ignored.
	 *
	 * @param  v  Value array.
	 *
	 * @return  This big integer.
	...
	/**
	 * Pack the value obtained from a portion of the given integer array into
	 * this big integer in little-endian order. Bits 0 through 31 of this big
	 * integer are set to <TT>v[off]</TT>, bits 32 through 63 of this big
	 * integer are set to <TT>v[off+1]</TT>, and so on. If <TT>len</TT> is not
	 * enough integers to fill this big integer, the unfilled most significant
	 * bits of this big integer are set to 0. If <TT>len</TT> is more integers
	 * than needed to fill this big integer, the extra integers are ignored.
	 *
	 * @param  v    Value array.
	 * @param  off  Index of first integer to pack.
	 * @param  len  Number of integers to pack.
	 *
	 * @return  This big integer.
	 *
	 * @exception  NullPointerException
	 ...
	 * <TT>v[1]</TT>, and so on. If <TT>v.length</TT> is not enough longs to
	 * fill this big integer, the unfilled most significant bits of this big
	 * integer are set to 0. If <TT>v.length</TT> is more longs than needed to
	 * fill this big integer, the extra longs are ignored.
	 *
	 * @param  v  Value array.
	 *
	 * @return  This big integer.
	...
	/**
	 * Pack the value obtained from a portion of the given long array into this
	 * big integer in little-endian order. Bits 0 through 63 of this big integer
	 * are set to <TT>v[off]</TT>, bits 64 through 127 of this big integer are
	 * set to <TT>v[off+1]</TT>, and so on. If <TT>len</TT> is not enough longs
	 * to fill this big integer, the unfilled most significant bits of this big
	 * integer are set to 0. If <TT>len</TT> is more longs than needed to fill
	 * this big integer, the extra longs are ignored.
	 *
	 * @param  v    Value array.
	 * @param  off  Index of first long to pack.
	 * @param  len  Number of longs to pack.
	 *
	 * @return  This big integer.
	 *
	 * @exception  NullPointerException
	 ...
	 * big integer are set to <TT>v[L-2]</TT>, and so on. If <TT>v.length</TT>
	 * is not enough bytes to fill this big integer, the unfilled most
	 * significant bits of this big integer are set to 0. If <TT>v.length</TT>
	 * is more bytes than needed to fill this big integer, the extra bytes are
	 * ignored.
	 *
	 * @param  v  Value array.
	 *
	...
	public BigInteger packBigEndian
		(byte[] v)
		{
		return packBigEndian (v, 0, v.length);
		}

	/**
	 * Pack the value obtained from a portion of the given byte array into this
	 * big integer in big-endian order. Bits 0 through 7 of this big integer are
	 * set to <TT>v[off+len-1]</TT>, bits 8 through 15 of this big integer are
	 * set to <TT>v[off+len-2]</TT>, and so on. If <TT>len</TT> is not enough
	 * bytes to fill this big integer, the unfilled most significant bits of
	 * this big integer are set to 0. If <TT>len</TT> is more bytes than needed
	 * to fill this big integer, the extra bytes are ignored.
	 *
	 * @param  v    Value array.
	 * @param  off  Index of first byte to pack.
	 * @param  len  Number of bytes to pack.
	...
	public BigInteger packBigEndian
		...
		int src = max - 1;
		int dst = 0;
		while (src >= off && dst < value.length)
			{
			value[dst] = v[src--] & 0xff;
			if (src >= off) value[dst] |= (v[src--] & 0xff) << 8;
			if (src >= off) value[dst] |= (v[src--] & 0xff) << 16;
			if (src >= off) value[dst] |= (v[src--] & 0xff) << 24;
			++ dst;
			}
		clearWords (value, dst);
		clearUnusedBits();
	 ...
	 * this big integer are set to <TT>v[L-2]</TT>, and so on. If
	 * <TT>v.length</TT> is not enough shorts to fill this big integer, the
	 * unfilled most significant bits of this big integer are set to 0. If
	 * <TT>v.length</TT> is more shorts than needed to fill this big integer,
	 * the extra shorts are ignored.
	 *
	 * @param  v  Value array.
	 *
	 * @return  This big integer.
	...
	/**
	 * Pack the value obtained from a portion of the given short array into this
	 * big integer in big-endian order. Bits 0 through 15 of this big integer
	 * are set to <TT>v[off+len-1]</TT>, bits 15 through 31 of this big integer
	 * are set to <TT>v[off+len-2]</TT>, and so on. If <TT>len</TT> is not
	 * enough shorts to fill this big integer, the unfilled most significant
	 * bits of this big integer are set to 0. If <TT>len</TT> is more shorts
	 * than needed to fill this big integer, the extra shorts are ignored.
	 *
	 * @param  v    Value array.
	 * @param  off  Index of first short to pack.
	 * @param  len  Number of shorts to pack.
	...
	public BigInteger packBigEndian
		...
		while (src >= off && dst < value.length)
			{
			value[dst] = v[src--] & 0xffff;
			if (src >= off) value[dst] |= (v[src--] & 0xffff) << 16;
			++ dst;
			}
		clearWords (value, dst);
		clearUnusedBits();
	 ...
	 * <TT>v.length</TT> is not enough integers to fill this big integer, the
	 * unfilled most significant bits of this big integer are set to 0. If
	 * <TT>v.length</TT> is more integers than needed to fill this big integer,
	 * the extra integers are ignored.
	 *
	 * @param  v  Value array.
	 *
	 * @return  This big integer.
	...
	/**
	 * Pack the value obtained from a portion of the given integer array into
	 * this big integer in big-endian order. Bits 0 through 31 of this big
	 * integer are set to <TT>v[off+len-1]</TT>, bits 32 through 63 of this big
	 * integer are set to <TT>v[off+len-2]</TT>, and so on. If <TT>len</TT> is
	 * not enough integers to fill this big integer, the unfilled most
	 * significant bits of this big integer are set to 0. If <TT>len</TT> is
	 * more integers than needed to fill this big integer, the extra integers
	 * are ignored.
	 *
	 * @param  v    Value array.
	 * @param  off  Index of first integer to pack.
	 ...
	 * this big integer are set to <TT>v[L-2]</TT>, and so on. If
	 * <TT>v.length</TT> is not enough longs to fill this big integer, the
	 * unfilled most significant bits of this big integer are set to 0. If
	 * <TT>v.length</TT> is more longs than needed to fill this big integer, the
	 * extra longs are ignored.
	 *
	 * @param  v  Value array.
	 *
	 * @return  This big integer.
	...
	/**
	 * Pack the value obtained from a portion of the given long array into this
	 * big integer in big-endian order. Bits 0 through 63 of this big integer
	 * are set to <TT>v[off+len-1]</TT>, bits 64 through 127 of this big integer
	 * are set to <TT>v[off+len-2]</TT>, and so on. If <TT>len</TT> is not
	 * enough longs to fill this big integer, the unfilled most significant bits
	 * of this big integer are set to 0. If <TT>len</TT> is more longs than
	 * needed to fill this big integer, the extra longs are ignored.
	 *
	 * @param  v    Value array.
	 * @param  off  Index of first long to pack.
	 * @param  len  Number of longs to pack.
	 ...
	 * so on. If there are fewer than <TT>v.length</TT> bytes in this big
	 * integer's value, the unfilled portion of <TT>v</TT> is set to 0. If there
	 * are more than <TT>v.length</TT> bytes in this big integer's value, the
	 * extra bytes in this big integer's value are ignored.
	 *
	 * @param  v  Value array.
	 *
	 * @return  This big integer.
	...
	public BigInteger unpack
		(byte[] v)
		{
		return unpack (v, 0, v.length);
		}

	/**
	 * Unpack this big integer's value into a portion of the given byte array in
	 * little-endian order. <TT>v[off]</TT> is set to bits 0&ndash;7 of this big
	 * integer, <TT>v[off+1]</TT> is set to bits 8&ndash;15 of this big integer,
	 * and so on. If there are fewer than <TT>len</TT> bytes in this big
	 * integer's value, the unfilled portion of <TT>v</TT> is set to 0. If there
	 * are more than <TT>len</TT> bytes in this big integer's value, the extra
	 * bytes in this big integer's value are ignored.
	 *
	 * @param  v    Value array.
	 * @param  off  Index of first byte to unpack.
	 * @param  len  Number of bytes to unpack.
	 *
	 * @return  This big integer.
	 *
	 * @exception  NullPointerException
	...
	public BigInteger unpack
		...
		int src = 0;
		int dst = off;
		while (src < value.length && dst < max)
			{
			v[dst++] = (byte)value[src];
			if (dst < max) v[dst++] = (byte)(value[src] >>> 8);
			if (dst < max) v[dst++] = (byte)(value[src] >>> 16);
			if (dst < max) v[dst++] = (byte)(value[src] >>> 24);
			++ src;
			}
		while (dst < max)
			v[dst++] = (byte)0;
	 ...
	 * and so on. If there are fewer than <TT>v.length</TT> shorts in this big
	 * integer's value, the unfilled portion of <TT>v</TT> is set to 0. If there
	 * are more than <TT>v.length</TT> shorts in this big integer's value, the
	 * extra shorts in this big integer's value are ignored.
	 *
	 * @param  v  Value array.
	 *
	 * @return  This big integer.
	...
	/**
	 * Unpack this big integer's value into a portion of the given short array
	 * in little-endian order. <TT>v[off]</TT> is set to bits 0&ndash;15 of this
	 * big integer, <TT>v[off+1]</TT> is set to bits 16&ndash;31 of this big
	 * integer, and so on. If there are fewer than <TT>len</TT> shorts in this
	 * big integer's value, the unfilled portion of <TT>v</TT> is set to 0. If
	 * there are more than <TT>len</TT> shorts in this big integer's value, the
	 * extra shorts in this big integer's value are ignored.
	 *
	 * @param  v    Value array.
	 * @param  off  Index of first short to unpack.
	 * @param  len  Number of shorts to unpack.
	 *
	 * @return  This big integer.
	 *
	 * @exception  NullPointerException
	...
	public BigInteger unpack
		...
		while (src < value.length && dst < max)
			...
			if (dst < max) v[dst++] = (short)(value[src] >>> 16);
			++ src;
			}
		while (dst < max)
			v[dst++] = (short)0;
	 ...
	 * and so on. If there are fewer than <TT>v.length</TT> integers in this big
	 * integer's value, the unfilled portion of <TT>v</TT> is set to 0. If there
	 * are more than <TT>v.length</TT> integers in this big integer's value, the
	 * extra integers in this big integer's value are ignored.
	 *
	 * @param  v  Value array.
	 *
	 * @return  This big integer.
	...
	/**
	 * Unpack this big integer's value into a portion of the given integer array
	 * in little-endian order. <TT>v[off]</TT> is set to bits 0&ndash;31 of this
	 * big integer, <TT>v[off+1]</TT> is set to bits 32&ndash;63 of this big
	 * integer, and so on. If there are fewer than <TT>len</TT> integers in this
	 * big integer's value, the unfilled portion of <TT>v</TT> is set to 0. If
	 * there are more than <TT>len</TT> integers in this big integer's value,
	 * the extra integers in this big integer's value are ignored.
	 *
	 * @param  v    Value array.
	 * @param  off  Index of first integer to unpack.
	 * @param  len  Number of integers to unpack.
	 *
	 * @return  This big integer.
	 *
	 * @exception  NullPointerException
	 ...
	 * and so on. If there are fewer than <TT>v.length</TT> longs in this big
	 * integer's value, the unfilled portion of <TT>v</TT> is set to 0. If there
	 * are more than <TT>v.length</TT> longs in this big integer's value, the
	 * extra longs in this big integer's value are ignored.
	 *
	 * @param  v  Value array.
	 *
	 * @return  This big integer.
	...
	/**
	 * Unpack this big integer's value into a portion of the given long array in
	 * little-endian order. <TT>v[off]</TT> is set to bits 0&ndash;63 of this
	 * big integer, <TT>v[off+1]</TT> is set to bits 64&ndash;127 of this big
	 * integer, and so on. If there are fewer than <TT>len</TT> longs in this
	 * big integer's value, the unfilled portion of <TT>v</TT> is set to 0. If
	 * there are more than <TT>len</TT> longs in this big integer's value, the
	 * extra longs in this big integer's value are ignored.
	 *
	 * @param  v    Value array.
	 * @param  off  Index of first long to unpack.
	 * @param  len  Number of longs to unpack.
	 *
	 * @return  This big integer.
	 *
	 * @exception  NullPointerException
	 ...
	 * of this big integer, and so on. If there are fewer than <TT>v.length</TT>
	 * bytes in this big integer's value, the unfilled portion of <TT>v</TT> is
	 * set to 0. If there are more than <TT>v.length</TT> bytes in this big
	 * integer's value, the extra bytes in this big integer's value are ignored.
	 *
	 * @param  v  Value array.
	 *
	 * @return  This big integer.
	...
	public BigInteger unpackBigEndian
		(byte[] v)
		{
		return unpackBigEndian (v, 0, v.length);
		}

	/**
	 * Unpack this big integer's value into a portion of the given byte array in
	 * big-endian order. <TT>v[off+len-1]</TT> is set to bits 0&ndash;7 of this
	 * big integer, <TT>v[off+len-2]</TT> is set to bits 8&ndash;15 of this big
	 * integer, and so on. If there are fewer than <TT>len</TT> bytes in this
	 * big integer's value, the unfilled portion of <TT>v</TT> is set to 0. If
	 * there are more than <TT>len</TT> bytes in this big integer's value, the
	 * extra bytes in this big integer's value are ignored.
	 *
	 * @param  v    Value array.
	 * @param  off  Index of first byte to unpack.
	...
	public BigInteger unpackBigEndian
		...
		int src = 0;
		int dst = max - 1;
		while (src < value.length && dst >= off)
			{
			v[dst--] = (byte)value[src];
			if (dst >= off) v[dst--] = (byte)(value[src] >>> 8);
			if (dst >= off) v[dst--] = (byte)(value[src] >>> 16);
			if (dst >= off) v[dst--] = (byte)(value[src] >>> 24);
			++ src;
			}
		while (dst >= off)
			v[dst--] = (byte)0;
	 ...
	 * 16&ndash;31 of this big integer, and so on. If there are fewer than
	 * <TT>v.length</TT> shorts in this big integer's value, the unfilled
	 * portion of <TT>v</TT> is set to 0. If there are more than
	 * <TT>v.length</TT> shorts in this big integer's value, the extra shorts in
	...
	/**
	 * Unpack this big integer's value into a portion of the given short array
	 * in big-endian order. <TT>v[off+len-1]</TT> is set to bits 0&ndash;15 of
	 * this big integer, <TT>v[off+len-2]</TT> is set to bits 16&ndash;31 of
	 * this big integer, and so on. If there are fewer than <TT>len</TT> shorts
	 * in this big integer's value, the unfilled portion of <TT>v</TT> is set to
	 * 0. If there are more than <TT>len</TT> shorts in this big integer's
	 * value, the extra shorts in this big integer's value are ignored.
	 *
	 * @param  v    Value array.
	 * @param  off  Index of first short to unpack.
	 * @param  len  Number of shorts to unpack.
	...
	public BigInteger unpackBigEndian
		...
		while (src < value.length && dst >= off)
			{
			v[dst--] = (short)value[src];
			if (dst >= off) v[dst--] = (short)(value[src] >>> 16);
			++ src;
			}
		while (dst >= off)
			v[dst--] = (short)0;
	...
	/**
	 * Unpack this big integer's value into the given integer array
	 * in big-endian order. Let <TT>L</TT> = <TT>v.length</TT>. <TT>v[L-1]</TT>
	 * is set to bits 0&ndash;31 of this big integer, <TT>v[L-2]</TT> is set to
	 * bits 32&ndash;63 of this big integer, and so on. If there are fewer than
	 * <TT>v.length</TT> integers in this big integer's value, the unfilled
	 * portion of <TT>v</TT> is set to 0. If there are more than
	 * <TT>v.length</TT> integers in this big integer's value, the extra
	 * integers in this big integer's value are ignored.
	 *
	 * @param  v    Value array.
	 *
	 * @return  This big integer.
	...
	/**
	 * Unpack this big integer's value into a portion of the given integer array
	 * in big-endian order. <TT>v[off+len-1]</TT> is set to bits 0&ndash;31 of
	 * this big integer, <TT>v[off+len-2]</TT> is set to bits 32&ndash;63 of
	 * this big integer, and so on. If there are fewer than <TT>len</TT>
	 * integers in this big integer's value, the unfilled portion of <TT>v</TT>
	 * is set to 0. If there are more than <TT>len</TT> integers in this big
	 * integer's value, the extra integers in this big integer's value are
	 * ignored.
	 *
	 * @param  v    Value array.
	 * @param  off  Index of first integer to unpack.
	 ...
	 * 64&ndash;127 of this big integer, and so on. If there are fewer than
	 * <TT>v.length</TT> longs in this big integer's value, the unfilled portion
	 * of <TT>v</TT> is set to 0. If there are more than <TT>v.length</TT> longs
	 * in this big integer's value, the extra longs in this big integer's value
	 * are ignored.
	 *
	 * @param  v  Value array.
	 *
	...
	/**
	 * Unpack this big integer's value into a portion of the given long array in
	 * big-endian order. <TT>v[off+len-1]</TT> is set to bits 0&ndash;63 of this
	 * big integer, <TT>v[off+len-2]</TT> is set to bits 64&ndash;127 of this
	 * big integer, and so on. If there are fewer than <TT>len</TT> longs in
	 * this big integer's value, the unfilled portion of <TT>v</TT> is set to 0.
	 * If there are more than <TT>len</TT> longs in this big integer's value,
	 * the extra longs in this big integer's value are ignored.
	 *
	 * @param  v    Value array.
	 * @param  off  Index of first long to unpack.
	 * @param  len  Number of longs to unpack.
...
// Hidden operations.
	...
	private void clearUnusedBits()
		{
		clearUnusedBits (value);
		}

	/**
	 * Clear the unused high-order bits in the destination.
	 */
	private void clearUnusedBits
		(int[] dst)
		{
...
//		{
//		System.err.println ("Usage: java edu.rit.numeric.BigInteger <n> <a> <b>");
//		System.exit (1);
//		}
...
//	private static void usage()
//		{
//		System.err.println ("Usage: java edu.rit.numeric.BigInteger <file> <n> <a>");
//		System.exit (1);
//		}
...
//		BigInteger x = new BigInteger (bits) .assign (args[1]);
//		int n = Integer.parseInt (args[2]);
//		BigInteger p = new BigInteger (x) .pow (n);
//		System.out.printf ("%s^%d = %s%n", x, n, p);
//		}
//
//	/**
//	 * Print a usage message and exit.
...
//	private static void usage()
//		{
//		System.err.println ("Usage: java edu.rit.numeric.BigInteger <bits> <x> <n>");
//		System.exit (1);
//		}
