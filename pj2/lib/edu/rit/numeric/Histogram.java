...
// Exported operations.
	...
	/**
	 * Returns the log Bayes factor for a Bayesian model selection for this
	 * histogram. Hypothesis <I>H</I><SUB>1</SUB> is that the observed bin
	 * counts obey the expected distribution, where the expected count in bin
	 * <I>i</I> is determined by calling the {@link #expectedCount(int)
	 * expectedCount(i)} method. Hypothesis <I>H</I><SUB>2</SUB> is that the
	 * observed bin counts do not obey the expected distribution. Assuming the
	 * prior odds ratio of the hypotheses is 1, a log Bayes factor &gt; 0
	 * indicates that the bin counts support <I>H</I><SUB>1</SUB> more than
	 * <I>H</I><SUB>2</SUB>, and a log Bayes factor &lt; 0 indicates the
	 * opposite.
	 *
	...
	public double logBayesFactor()
		{
		// Find the bin whose observed cumulative distribution is farthest away
		// from its expected cumulative distribution.
		long actual = 0;
		double expect = 0.0;
		double diff;
		double maxdiff = 0.0;
		double p = 0.0;
		long k = 0;
		for (int i = 0; i < B - 1; ++ i) // Omit last bin
			{
			actual += count[i];
			expect += expectedCount (i);
		...
		return Statistics.bayesianBinomialTest (total, p, k);
		}
