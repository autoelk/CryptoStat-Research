...
// Exported operations.
	 ...
	 * in the range 0 .. 63, false is returned.
	 *
	 * @param  elem  Element.
	 *
	 ...
	 *
	 * @param  prng  Pseudorandom number generator.
	 * @param  N     The set can contain elements from 0 through
	 *               <TT>N</TT>&minus;1.
	 *
	 * @return  Random non-element, or &minus;1 if this set is full.
	 *
	 * @exception  IllegalArgumentException
	 *     (unchecked exception) Thrown if <TT>N</TT> &lt; 0 or <TT>N</TT> &gt;
	 *     64.
	 */
	public int randomNonElement
		...
		if (N < 0 || N > 64)
			throw new IllegalArgumentException (String.format
				("BitSet64.randomNonElement(): N = %d illegal", N));

		int n = N - Long.bitCount (bitmap);
		if (n <= 0) return -1;
		int i = prng.nextInt (n);
		int e = 0;
		for (;;)
	...
	public int[] toArray
		...
		int shift = 0;
		int n = 0;
		while (shift < 64 && n < len)
			{
			if (((bitmap >> shift) & 1L) == 1L)
				{
				array [off+n] = shift;
				++ n;
				}
