...
// Unit:    Class edu.rit.util.MultiSet
//
...
/**
 * Class MultiSet provides a multiset of elements. A multiset is a set in which
 * each element may appear more than once. An element's <I>multiplicity</I> (an
 * integer greater than or equal to 0) is the number of times the element
 * appears in the multiset. The element must be an object suitable for use in a
 * hashed data structure; that is, the element class must define the {@link
 * Object#equals(Object) equals()} and {@link Object#hashCode() hashCode()}
 * methods properly. Elements must not be null. Once an element is stored in a
 * multiset, the state of the element must not be altered.
 * <P>
 * The multiset calls the protected {@link #getPair(Object,int) getPair()}
 ...
 * overriding the aforementioned methods in a subclass of class MultiSet.
 * <P>
 * Class MultiSet is streamable. For object streaming to work properly, the
 * element class must be streamable or serializable.
 * <P>
 * <I>Note:</I> Class MultiSet is not multiple thread safe.
 *
...
// Hidden data members.
	...
	private Set<IntPair<E>> elements = new Set<IntPair<E>>()
		{
		protected IntPair<E> copyElement (IntPair<E> elem)
			{
...
// Exported constructors.
	 ...
	 * {@link #getPair(Object,int) getPair()} method.
	 *
...
// Exported operations.
	...
	public int countAll()
		{
		return elements.forEachItemDo (new ActionResult<IntPair<E>,Integer>()
			{
			int total;
			public void run (IntPair<E> pair)
				{
				total += pair.value();
				}
	...
	public boolean contains
		(E elem)
		{
		return elements.contains (new IntPair<E> (elem, 0));
		}
	...
	public int count
		(E elem)
		{
		IntPair<E> pair = elements.get (new IntPair<E> (elem, 0));
	 ...
	 * {@link #getPair(Object,int) getPair()} method, is added to this multiset.
	 *
	 * @param  elem  Element.
	 *
	 * @exception  NullPointerException
	...
	public void add
		...
		if (count < 0)
			throw new IllegalArgumentException (String.format
				("MultiSet.add(): count = %d illegal", count));
		IntPair<E> newPair = new IntPair<E> (elem, count);
		IntPair<E> oldPair = elements.get (newPair);
		if (oldPair == null)
	...
	public void remove
		...
		if (count < 0)
			throw new IllegalArgumentException (String.format
				("MultiSet.remove(): count = %d illegal", count));
		IntPair<E> pair = elements.get (new IntPair<E> (elem, 0));
		if (pair == null)
			{ }
	...
	public void removeAll
		(E elem)
		{
		elements.remove (new IntPair<E> (elem, 0));
		}
	...
	public boolean isSubsetOf
		(final MultiSet<E> mset)
		{
		return elements.forEachItemDo (new ActionResult<IntPair<E>,Boolean>()
			{
	...
	public void union
		(MultiSet<E> mset)
		{
		mset.elements.forEachItemDo (new Action<IntPair<E>>()
			{
			public void run (IntPair<E> msetpair)
				{
	...
	public void intersection
		(final MultiSet<E> mset)
		{
		this.elements.removeEachItemIf (new Predicate<IntPair<E>>()
			{
			public boolean test (IntPair<E> thispair)
				{
				thispair.value
	...
	public void symmetricDifference
		...
		mset.elements.forEachItemDo (new Action<IntPair<E>>()
			...
			public void run (IntPair<E> msetpair)
				{
				IntPair<E> thispair = elements.get (msetpair);
				if (thispair == null)
					add (msetpair.key(), msetpair.value());
	...
	public void forEachItemDo
		(final Action<IntPair<E>> action)
		{
		elements.removeEachItemIf (new Predicate<IntPair<E>>()
			{
			public boolean test (IntPair<E> pair)
				{
				action.run (pair);
				return pair.value() <= 0;
				}
	...
	public <R> R forEachItemDo
		(final ActionResult<IntPair<E>,R> action)
		{
		elements.removeEachItemIf (new Predicate<IntPair<E>>()
			{
			public boolean test (IntPair<E> pair)
	 ...
	 * writeReference()}. The elements' multiplicities are written using {@link
	 * OutStream#writeInt(int) writeInt()}.
	 *
	 ...
	 * are read using {@link InStream#readReference() readReference()}. The
	 * elements' multiplicities are written using {@link InStream#readInt()
	 * readInt()}.
	 *
	 * @param  in  In stream.
	 *
...
// Hidden operations.
	 ...
	 * class MultiSet needs to create a pair, it does so by calling this method.
	 * <P>
	 * The <TT>getPair()</TT> method in class MultiSet returns an instance of
	 * class {@linkplain IntPair IntPair}&lt;E&gt;. The given element is copied
	 * using the {@link #copyElement(Object) copyElement()} method. A subclass
	 * of class MultiSet can override the <TT>getPair()</TT> method to return
	 * something else. Possibilities include:
	 * <UL>
	 * <P><LI>
	 * Return an instance of a subclass of class {@link IntPair
	 * IntPair}&lt;E&gt;, in which the {@link Object#equals(Object) equals()}
	 * and {@link Object#hashCode() hashCode()} methods have been overridden to
	 * implement a different pair equality criterion.
	 * <P><LI>
	 * Return an instance of a subclass of class {@link IntPair
	 * IntPair}&lt;E&gt;, in which the {@link Streamable#writeOut(OutStream)
	 ...
	 */
	protected IntPair<E> getPair
		(E key,
		 int value)
		{
		return new IntPair<E> (copyElement (key), value);
		}

	/**
	 * Copy the given element. The <TT>copyElement()</TT> method in class
	 * MultiSet merely returns the <TT>elem</TT> reference; this makes a
	 * <I>shallow copy.</I> A subclass of class MultiSet can override the
