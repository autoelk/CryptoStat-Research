 ...
 * the job or task should call the <TT>terminate()</TT> method. Code in a job or
	...
	public static void main
		...
		try
			...
			if (Task.class.isAssignableFrom (jtclass))
				...
				// For a NativeTask, do not do multiple thread safe console
				// output.
				safe = ! (task instanceof NativeTask);
				}
			...
			// Execute job.
			job.main (taskArgs);
			System.exit (job.execute (safe));
			}

		// On a TerminateException, terminate with the proper exit code.
		catch (TerminateException exc)
			{
			System.exit (exc.exitCode);
			}

		// On any other exception, print stack trace and terminate.
		catch (Throwable exc)
			{
