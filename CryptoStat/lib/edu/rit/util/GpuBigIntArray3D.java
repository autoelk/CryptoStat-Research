//******************************************************************************
//
// File:    GpuBigIntArray3D.java
// Package: edu.rit.util
// Unit:    Class edu.rit.util.GpuBigIntArray3D
//
...
/**
 * Class GpuBigIntArray3D provides a 3-D array of {@linkplain BigInt}s located
 * on the GPU and mirrored on the CPU.
 *
 * @author  Alan Kaminsky
 * @version 23-Aug-2017
 */
public class GpuBigIntArray3D
	{

// Exported data members.

	/**
	 * 3-D array of {@linkplain BigInt}s.
	 */
...
// Exported constructors.
	...
	/**
	 * Construct a new bigint 3-D array with the given dimensions. Each element
	 ...
	 * @param  len3     Array length along third dimension.
	 *
	 * @exception  IllegalArgumentException
	 *     (unchecked exception) Thrown if <TT>bitSize</TT> &lt; 0.
	 * @exception  NegativeArraySizeException
	 *     (unchecked exception) Thrown if <TT>len1</TT> &lt; 0. Thrown if
	 *     <TT>len2</TT> &lt; 0. Thrown if <TT>len3</TT> &lt; 0.
	 */
	public GpuBigIntArray3D
		(int bitSize,
		 int len1,
		 int len2,
		 int len3)
		{
		this.item = new BigInt [len1] [len2] [len3];
		for (int i = 0; i < len1; ++ i)
			for (int j = 0; j < len2; ++ j)
				for (int k = 0; k < len3; ++ k)
					this.item[i][j][k] = new BigInt (bitSize);
...
// Exported operations.
	...
	/**
	 * Clear this bigint 3-D array. All elements are set to 0.
	 */
	public void clear()
		{
		for (int i = 0; i < len1; ++ i)
	...
	/**
	 * Allocate storage for this bigint 3-D array in global memory on the GPU.
	 * This bigint 3-D array will mirror the allocated storage.
	 *
	...
	public void allocate
		(Gpu gpu)
		{
		array = gpu.getIntArray (len1*len2*len3*wordSize);
		}

	/**
	 * Set this bigint 3-D array to mirror the given global variable on the GPU.
	 * <P>
	 * <I>Note:</I> It is assumed that the global variable is a uint32_t array
	 * with enough elements to hold this bigint 3-D array.
	 *
	...
	public void mirror
		(Module module,
		 String name)
		{
		array = module.getIntArray (name, len1*len2*len3*wordSize);
		}
	...
	/**
	 * Copy this GPU bigint 3-D array from the host CPU's memory to the given
	 * GPU device's memory.
	 *
	 * @exception  NullPointerException
	 *     (unchecked exception) Thrown if the <TT>allocate()</TT> or
	...
	public void hostToDev()
		...
		for (int i = 0; i < len1; ++ i)
			for (int j = 0; j < len2; ++ j)
				for (int k = 0; k < len3; ++ k)
					{
					item[i][j][k].unpack (array.item, off);
					off += wordSize;
					}
		array.hostToDev();
		}
	...
	/**
	 * Copy this GPU bigint 3-D array from the given GPU device's memory to the
	 * host CPU's memory.
	 *
	 * @exception  NullPointerException
	 *     (unchecked exception) Thrown if the <TT>allocate()</TT> or
	...
	public void devToHost()
		...
		for (int i = 0; i < len1; ++ i)
			for (int j = 0; j < len2; ++ j)
				for (int k = 0; k < len3; ++ k)
					{
					item[i][j][k].pack (array.item, off);
					off += wordSize;
					}
		}

	/**
	 * Returns the word size of the bigints in this GPU bigint 3-D array. This
	...
	/**
	 * Returns the bit size of the bigints in this GPU bigint 3-D array. This is
	...
	private void verifyArray()
		...
		if (array == null)
			throw new NullPointerException
				("GpuBigIntArray3D.verifyArray(): Not allocated on GPU");
		}
