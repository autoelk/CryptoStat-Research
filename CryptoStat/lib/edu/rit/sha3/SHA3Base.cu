...
__device__ void evaluate(
	...
	// If there's only one padding byte set it to the correct padding
	if(numPaddingBytes == 1) {
		W[8] = 0x8000000000000006L;
	} else {
		// First byte is 06
		W[8] = 0x0000000000000006L;
		// all bytes inbetween are 0s
		for(i = 9; i < (8 + numPaddingBytes - 2); i++) {
			W[i] = 0x0000000000000000L;
		}
		// Last byte is 80
		W[8 + numPaddingBytes - 1] = 0x8000000000000000L;
	}
	// Fill capacity with 0s
	for(i = (8 + numPaddingBytes); i < 25; i++) {
		W[i] = 0x0000000000000000L;
	}
	...
	for (int roundNum = 0; roundNum < 24; roundNum++) {
		...
		recordDigest (biElem3D (Csize, C, NB, R, a, b, roundNum),
			A00, A10, A20, A30, A40, A01, A11, A21);
	}
}
